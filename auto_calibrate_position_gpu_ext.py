...
prev=json.loads((ROOT/'rigorous_results'/'position_auto_calibration.json').read_text())
...
weights=[2.4,3.2,4.2,5.5,7.0,9.0]
def evalw(w):
 sc,gr,ind,r=build_renderer_v11_gpu(ROOT,seed=4545,proposal_weight=PW,position_weight=w,device=0,memory_limit_mb=4608);vf=ProtectedSlotVerifier();bad=slots=0;sent=[];opens=[];diffs=[];rng=random.Random(99)
 for spec in specs:
  p=stable_world(**spec);o=r.render(p);bad+=int(Counter(o['represented'])!=Counter(p));slots+=len(vf.inspect_render(o));sent+=o['sentences'];opens += [opening(sc,s) for s in o['sentences']];by={}
  for s,g in zip(o['sentences'],o['groups']):by.setdefault(g[0][1],[]).append(s)
  for ss in by.values():
   if len(ss)<2:continue
   a,_=gr.opening_position_score_tokens(sc.tokenize(ss[0]),True,sc.is_slot);b,_=gr.opening_position_score_tokens(sc.tokenize(rng.choice(ss[1:])),True,sc.is_slot);diffs.append(a-b)
 supports=[]
 ...
 return {'position_weight':w,'bad':bad,'slot_errors':slots,'support':statistics.mean(supports),'repeat':rep,'opening_entropy':entropy_norm(opens),'contrast_mean':statistics.mean(diffs),'contrast_win_rate':sum(x>0 for x in diffs)/len(diffs),'comparisons':len(diffs)}
for w in weights:
